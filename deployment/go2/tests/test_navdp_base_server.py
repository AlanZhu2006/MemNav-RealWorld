...
class FakeImageGoalNavigator:
    batch_size = 1

    def __init__(self):
        self.image_goal_shape = None

    def step_imagegoal(self, image_goal, rgb, depth):
...
class ImageGoalRouteTests(unittest.TestCase):
    def setUp(self):
        self.navigator = FakeImageGoalNavigator()
    ...
    def test_route_decodes_the_exact_imagegoal_contract(self):
        goal_buffer = io.BytesIO()
        Image.new("RGB", (8, 6), color=(20, 40, 60)).save(
            goal_buffer, format="PNG"
        )
        goal_buffer.seek(0)
        ...
        with mock.patch.object(
            ...
            response = self.client.post(
                "/imagegoal_step",
                data={"goal": (goal_buffer, "goal.png")},
