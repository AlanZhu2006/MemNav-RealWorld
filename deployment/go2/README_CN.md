...
不会直接铺开JSON。
由助手操作时，用户直接要求“启动 Revisit/导航”同样构成本轮运动授权；沿用已经说明的
现场安全条件，不再要求重复确认或固定口令。自动预检和故障锁停仍保留；修改、同步、
Prepare 与 Survey 不会自动触发运动，Stop 后必须等新的启动请求。
运行中 RGB-D 接收年龄或源帧年龄超过 2 秒时，自动归零并显示 `PAUSED · WAIT RGB-D`。
这是本轮内的暂停：放弃旧动作，等停下之后的新 RGB-D 和新规划就绪后继续，不重启整栈，
也不再次询问授权。一直不恢复则保持零速度，最终由原有运行总超时结束；人工 Stop、急停
和其他终止故障不自动恢复。首次启动预检仍要求画面足够新鲜（0.60 秒）。
规划或几何更新超过 5 秒时也改为本轮内停车等待，显示 `PAUSED · WAIT PLAN`，
不再仅因这个超时结束 Revisit。旧路径、旧转向目标和停车前尚未返回的规划会被丢弃；
只有停车后的新 RGB-D 生成且通过 3 秒输入年龄检查的规划才能恢复运动。
等待计入原有运行总预算，Stop、急停和其他终止故障仍然有效。该行为需要新版控制节点
和监督程序一起加载；监督程序对未声明恢复能力的旧控制节点仍保留原超时保护。
Revisit 操作服务的默认总预算为 500 秒，包含运行中的暂停等待时间。
Full-Mono Revisit 每次仅上传当前 RGB 和已安装 Goal 的 SHA-256；服务器严格校验
哈希并使用已保存的目标图像。固定 Goal 和 D435i 深度不再逐次上传，RGB 编码质量不变。
本地 D435i 深度仍供安全检查和实验记录使用，两个 paired arms 均使用 LingBot 深度。
两端通过 `installed_goal_rgb_only_supported` 能力检查确认上传协议兼容。
相机恢复服务仍保留在ROS侧供维护使用，但不再占用默认Operate操作面板。
Survey 启动通过常驻 ROS 客户端发现和调用服务，先确认服务可用再打开录制。
如果 Survey 尚未正式启动就发生系统故障，`START SURVEY` 可沿用已保存的目标重试：
目标文件按哈希校验后原样复制到新 Episode，原失败记录和录像保留，不会重开已封存录像。
停止原因与实验结果分开：自动到达、手动 `STOP`、超时或运行故障均只记停止原因，
自动保存后的结果为 `unreviewed`（待人工判定），不是自动成功/失败。GUI 不提供成功/失败
标注按钮，由操作员离线判定；手动停止也可以判为成功。可选的人工标注保存在该次
